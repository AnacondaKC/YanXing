...
export function DashboardView({
  ...
  return (
    <div className="flex min-h-0 min-w-0 flex-col gap-3 sm:gap-4 lg:min-h-0 lg:flex-1">
      <div className="yx-dashboard-grid grid min-h-0 min-w-0 flex-1 grid-cols-1 gap-3 sm:gap-4 xl:grid-cols-[minmax(0,1fr)_350px]">
        <div className="order-1 min-h-0 min-w-0 grid grid-cols-1 gap-3 sm:gap-4 xl:order-none xl:col-start-1 xl:col-span-1 xl:row-start-1 xl:grid-cols-2 xl:items-stretch xl:gap-4 xl:h-full">
          <AiScoreCard
            ...
            analyzing={analyzing}
            jobStatus={jobStatus}
            className="xl:h-[289px] xl:min-h-0 xl:overflow-hidden xl:!py-[20px] xl:!px-[25px]"
          />
          <ReportOverviewCard report={report} stageLabel={reportStageLabel} snapshot={snapshot} analyzing={analyzing} jobStatus={jobStatus} className="xl:h-[289px] xl:min-h-0 xl:overflow-hidden xl:!py-[20px] xl:!px-[25px]" />
        </div>
        <div className="order-3 min-h-0 min-w-0 flex flex-col gap-3 xl:order-none xl:col-start-2 xl:col-span-1 xl:row-start-1 xl:row-span-2 xl:w-[350px] xl:self-start xl:grid xl:grid-cols-1 xl:grid-rows-[subgrid] xl:gap-4 xl:h-full">
          <div className="flex min-w-0 flex-col gap-3 xl:row-start-1 xl:h-full xl:min-h-0 xl:gap-[5px] xl:w-[350px]">
            <ResearchWorkbench className="xl:shrink-0" analyzing={analyzing} cancelling={cancelling} job={job} moduleStates={moduleStates} canManage={canManage} report={report} viewingHistoricalReport={viewingHistoricalReport} historicalStageName={reportMilestone?.title} onReturnToLatestReport={onReturnToLatestReport} onCancelAnalysis={onCancelAnalysis} onOpenProgress={onOpenProgress} onEditProject={onEditProject} onStartAnalysis={onStartAnalysis} />
            <ReportCompletenessCard snapshot={snapshot} analyzing={analyzing} jobStatus={jobStatus} className="w-full self-stretch overflow-hidden xl:h-[168px] xl:flex-none xl:!pt-[10px] xl:!pb-[20px]" />
          </div>
          <div className="min-h-0 min-w-0 xl:row-start-2 xl:self-stretch xl:w-[350px]">
            <AiSuggestionsCard snapshot={snapshot} analyzing={analyzing} jobStatus={jobStatus} className="xl:h-full xl:min-h-0 xl:!pt-[20px] xl:!pb-[20px]" />
          </div>
        </div>
        <ResearchVisualizationCard
          snapshot={snapshot}
          analyzing={analyzing}
          jobStatus={jobStatus}
          className="order-5 xl:order-none xl:col-start-1 xl:col-span-1 xl:row-start-2 xl:h-full"
